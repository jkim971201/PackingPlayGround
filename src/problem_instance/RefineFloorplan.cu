...
RefineFloorplan::RefineFloorplan(
  float x_min, float y_min, float x_max, float y_max,
  std::shared_ptr<Painter> painter,
  std::vector<Macro*>& movable_macros,
  std::vector<Net*>& nets,
  std::vector<Pin*>& pins) : ProblemInstance(painter)
{
  x_min_       = x_min;
  y_min_       = y_min;
  x_max_       = x_max;
...
void
RefineFloorplan::setNeedExport(bool flag)
{
  need_export_ = flag;
}

void
RefineFloorplan::scaleArea(float scale)
{
  area_scale_ = scale;
...
void
RefineFloorplan::updatePointAndGetGrad(const CudaVector<float>& var, CudaVector<float>& grad)
{
...
void
RefineFloorplan::updateWidthAndHeight(const CudaVector<float>& var)
{
  int num_thread = 64;
...
void 
RefineFloorplan::getInitialGrad(
...
void 
RefineFloorplan::clipToFeasibleSolution(CudaVector<float>& var)
{
  int num_thread = 64;
  int num_block_cell = (num_macro_ - 1 + num_thread) / num_thread;
...
void 
RefineFloorplan::updateParameters()
{
  hpwl_ = computeHpwl();
...
void
RefineFloorplan::printProgress(int iter) const
{
...
void 
RefineFloorplan::solveBgnCbk()
{
}

void 
RefineFloorplan::solveEndCbk(int iter, double runtime, const CudaVector<float>& macro_pos)
{
...
void 
RefineFloorplan::iterBgnCbk(int iter)
{
}

void 
RefineFloorplan::iterEndCbk(
...
bool 
RefineFloorplan::checkConvergence() const
{
...
float
RefineFloorplan::getSumOverlap() const
{
  return sum_overlap_area_;
}

float
RefineFloorplan::getHpwl() const
{
  return hpwl_;
}

void
RefineFloorplan::computeWirelengthSubGrad(const CudaVector<float>& pos, CudaVector<float>& wl_grad)
{
  int num_thread    = 64;
...
void 
RefineFloorplan::computeOverlapSubGrad(
...
void
RefineFloorplan::exportToDb(const CudaVector<float>& d_solution)
{
